...
func (ec *executionContext) resolveEntity(
	...
	case "UserAddress":
		resolverName, err := entityResolverNameForUserAddress(ctx, rep)
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "UserAddress": %w`, err)
		}
		switch resolverName {
		...
		case "findUserAddressByID":
			id0, err := ec.unmarshalNUUID2githubᚗcomᚋgoogleᚋuuidᚐUUID(ctx, rep["id"])
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findUserAddressByID(): %w`, err)
			}
			entity, err := ec.Resolvers.Entity().FindUserAddressByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "UserAddress": %w`, err)
			}

			return entity, nil
		}
	case "VendorAddress":
		resolverName, err := entityResolverNameForVendorAddress(ctx, rep)
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "VendorAddress": %w`, err)
		}
		switch resolverName {
		...
		case "findVendorAddressByID":
			id0, err := ec.unmarshalNUUID2githubᚗcomᚋgoogleᚋuuidᚐUUID(ctx, rep["id"])
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findVendorAddressByID(): %w`, err)
			}
			entity, err := ec.Resolvers.Entity().FindVendorAddressByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "VendorAddress": %w`, err)
			}
...
func entityResolverNameForUserAddress(ctx context.Context, rep EntityRepresentation) (string, error) {
	// we collect errors because a later entity resolver may work fine
	// when an entity has multiple keys
	entityResolverErrs := []error{}
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for UserAddress", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for UserAddress", ErrTypeNotFound))
			break
		}
		return "findUserAddressByID", nil
	}
	return "", fmt.Errorf("%w for UserAddress due to %v", ErrTypeNotFound,
		errors.Join(entityResolverErrs...).Error())
}

func entityResolverNameForVendorAddress(ctx context.Context, rep EntityRepresentation) (string, error) {
	// we collect errors because a later entity resolver may work fine
	// when an entity has multiple keys
	entityResolverErrs := []error{}
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for VendorAddress", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for VendorAddress", ErrTypeNotFound))
			break
		}
		return "findVendorAddressByID", nil
	}
	return "", fmt.Errorf("%w for VendorAddress due to %v", ErrTypeNotFound,
		errors.Join(entityResolverErrs...).Error())
}
