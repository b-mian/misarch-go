...
func (ec *executionContext) resolveEntity(
	...
	case "Coupon":
		...
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "Coupon": %w`, err)
		}
		switch resolverName {
		...
		case "findCouponByID":
			id0, err := ec.unmarshalNUUID2githubᚗcomᚋgoogleᚋuuidᚐUUID(ctx, rep["id"])
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findCouponByID(): %w`, err)
			}
			entity, err := ec.Resolvers.Entity().FindCouponByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "Coupon": %w`, err)
			}

			return entity, nil
		}
	case "Discount":
		resolverName, err := entityResolverNameForDiscount(ctx, rep)
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "Discount": %w`, err)
		}
		switch resolverName {
		...
		case "findDiscountByID":
			id0, err := ec.unmarshalNUUID2githubᚗcomᚋgoogleᚋuuidᚐUUID(ctx, rep["id"])
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findDiscountByID(): %w`, err)
			}
			entity, err := ec.Resolvers.Entity().FindDiscountByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "Discount": %w`, err)
			}

			return entity, nil
		}
	case "DiscountUsage":
		resolverName, err := entityResolverNameForDiscountUsage(ctx, rep)
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "DiscountUsage": %w`, err)
		}
		switch resolverName {
		...
		case "findDiscountUsageByID":
			id0, err := ec.unmarshalNUUID2githubᚗcomᚋgoogleᚋuuidᚐUUID(ctx, rep["id"])
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findDiscountUsageByID(): %w`, err)
			}
			entity, err := ec.Resolvers.Entity().FindDiscountUsageByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "DiscountUsage": %w`, err)
			}
...
func entityResolverNameForCoupon(ctx context.Context, rep EntityRepresentation) (string, error) {
	// we collect errors because a later entity resolver may work fine
	// when an entity has multiple keys
	entityResolverErrs := []error{}
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for Coupon", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for Coupon", ErrTypeNotFound))
			break
		}
		return "findCouponByID", nil
	}
	return "", fmt.Errorf("%w for Coupon due to %v", ErrTypeNotFound,
		errors.Join(entityResolverErrs...).Error())
}

func entityResolverNameForDiscount(ctx context.Context, rep EntityRepresentation) (string, error) {
	// we collect errors because a later entity resolver may work fine
	// when an entity has multiple keys
	entityResolverErrs := []error{}
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for Discount", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for Discount", ErrTypeNotFound))
			break
		}
		return "findDiscountByID", nil
	}
	return "", fmt.Errorf("%w for Discount due to %v", ErrTypeNotFound,
		errors.Join(entityResolverErrs...).Error())
}

func entityResolverNameForDiscountUsage(ctx context.Context, rep EntityRepresentation) (string, error) {
	// we collect errors because a later entity resolver may work fine
	// when an entity has multiple keys
	entityResolverErrs := []error{}
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for DiscountUsage", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for DiscountUsage", ErrTypeNotFound))
			break
		}
		return "findDiscountUsageByID", nil
	}
	return "", fmt.Errorf("%w for DiscountUsage due to %v", ErrTypeNotFound,
		errors.Join(entityResolverErrs...).Error())
}
