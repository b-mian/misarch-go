...
func (ec *executionContext) resolveEntity(
	...
	switch typeName {
	case "ProductItem":
		resolverName, err := entityResolverNameForProductItem(ctx, rep)
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "ProductItem": %w`, err)
		}
		switch resolverName {

		case "findProductItemByID":
			id0, err := ec.unmarshalNUUID2string(ctx, rep["id"])
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findProductItemByID(): %w`, err)
			}
			entity, err := ec.Resolvers.Entity().FindProductItemByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "ProductItem": %w`, err)
			}
...
func entityResolverNameForProductItem(ctx context.Context, rep EntityRepresentation) (string, error) {
	...
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for ProductItem", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for ProductItem", ErrTypeNotFound))
			break
		}
		return "findProductItemByID", nil
	}
	return "", fmt.Errorf("%w for ProductItem due to %v", ErrTypeNotFound,
