...
func (ec *executionContext) resolveEntity(
	...
	case "Notification":
		...
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "Notification": %w`, err)
		}
		switch resolverName {
		...
		case "findNotificationByID":
			...
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findNotificationByID(): %w`, err)
			}
			entity, err := ec.Resolvers.Entity().FindNotificationByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "Notification": %w`, err)
			}
...
func entityResolverNameForNotification(ctx context.Context, rep EntityRepresentation) (string, error) {
	...
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for Notification", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for Notification", ErrTypeNotFound))
			break
		}
		return "findNotificationByID", nil
	}
	return "", fmt.Errorf("%w for Notification due to %v", ErrTypeNotFound,
