...
func (ec *executionContext) resolveEntity(
	...
	case "PaymentInformation":
		resolverName, err := entityResolverNameForPaymentInformation(ctx, rep)
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "PaymentInformation": %w`, err)
		}
		switch resolverName {
		...
		case "findPaymentInformationByID":
			id0, err := ec.unmarshalNUUID2githubᚗcomᚋgoogleᚋuuidᚐUUID(ctx, rep["id"])
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findPaymentInformationByID(): %w`, err)
			}
			entity, err := ec.Resolvers.Entity().FindPaymentInformationByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "PaymentInformation": %w`, err)
			}
...
func entityResolverNameForPaymentInformation(ctx context.Context, rep EntityRepresentation) (string, error) {
	// we collect errors because a later entity resolver may work fine
	// when an entity has multiple keys
	entityResolverErrs := []error{}
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for PaymentInformation", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for PaymentInformation", ErrTypeNotFound))
			break
		}
		return "findPaymentInformationByID", nil
	}
	return "", fmt.Errorf("%w for PaymentInformation due to %v", ErrTypeNotFound,
		errors.Join(entityResolverErrs...).Error())
}
