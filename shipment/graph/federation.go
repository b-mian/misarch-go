...
func (ec *executionContext) resolveEntity(
	...
	case "Return":
		resolverName, err := entityResolverNameForReturn(ctx, rep)
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "Return": %w`, err)
		}
		switch resolverName {
		...
		case "findReturnByID":
			id0, err := ec.unmarshalNUUID2githubᚗcomᚋgoogleᚋuuidᚐUUID(ctx, rep["id"])
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findReturnByID(): %w`, err)
			}
			entity, err := ec.Resolvers.Entity().FindReturnByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "Return": %w`, err)
			}

			return entity, nil
		}
	case "Shipment":
		resolverName, err := entityResolverNameForShipment(ctx, rep)
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "Shipment": %w`, err)
		}
		switch resolverName {
		...
		case "findShipmentByID":
			id0, err := ec.unmarshalNUUID2githubᚗcomᚋgoogleᚋuuidᚐUUID(ctx, rep["id"])
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findShipmentByID(): %w`, err)
			}
			entity, err := ec.Resolvers.Entity().FindShipmentByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "Shipment": %w`, err)
			}

			return entity, nil
		}
	case "ShipmentMethod":
		resolverName, err := entityResolverNameForShipmentMethod(ctx, rep)
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "ShipmentMethod": %w`, err)
		}
		switch resolverName {
		...
		case "findShipmentMethodByID":
			id0, err := ec.unmarshalNUUID2githubᚗcomᚋgoogleᚋuuidᚐUUID(ctx, rep["id"])
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findShipmentMethodByID(): %w`, err)
			}
			entity, err := ec.Resolvers.Entity().FindShipmentMethodByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "ShipmentMethod": %w`, err)
			}
...
func entityResolverNameForReturn(ctx context.Context, rep EntityRepresentation) (string, error) {
	// we collect errors because a later entity resolver may work fine
	// when an entity has multiple keys
	entityResolverErrs := []error{}
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for Return", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for Return", ErrTypeNotFound))
			break
		}
		return "findReturnByID", nil
	}
	return "", fmt.Errorf("%w for Return due to %v", ErrTypeNotFound,
		errors.Join(entityResolverErrs...).Error())
}

func entityResolverNameForShipment(ctx context.Context, rep EntityRepresentation) (string, error) {
	// we collect errors because a later entity resolver may work fine
	// when an entity has multiple keys
	entityResolverErrs := []error{}
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for Shipment", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for Shipment", ErrTypeNotFound))
			break
		}
		return "findShipmentByID", nil
	}
	return "", fmt.Errorf("%w for Shipment due to %v", ErrTypeNotFound,
		errors.Join(entityResolverErrs...).Error())
}

func entityResolverNameForShipmentMethod(ctx context.Context, rep EntityRepresentation) (string, error) {
	// we collect errors because a later entity resolver may work fine
	// when an entity has multiple keys
	entityResolverErrs := []error{}
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for ShipmentMethod", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for ShipmentMethod", ErrTypeNotFound))
			break
		}
		return "findShipmentMethodByID", nil
	}
	return "", fmt.Errorf("%w for ShipmentMethod due to %v", ErrTypeNotFound,
		errors.Join(entityResolverErrs...).Error())
}
