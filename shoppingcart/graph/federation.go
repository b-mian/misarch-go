...
func (ec *executionContext) resolveEntity(
	...
	case "ShoppingCartItem":
		resolverName, err := entityResolverNameForShoppingCartItem(ctx, rep)
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "ShoppingCartItem": %w`, err)
		}
		switch resolverName {
		...
		case "findShoppingCartItemByID":
			...
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findShoppingCartItemByID(): %w`, err)
			}
			entity, err := ec.Resolvers.Entity().FindShoppingCartItemByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "ShoppingCartItem": %w`, err)
			}
...
func entityResolverNameForShoppingCartItem(ctx context.Context, rep EntityRepresentation) (string, error) {
	...
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for ShoppingCartItem", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for ShoppingCartItem", ErrTypeNotFound))
			break
		}
		return "findShoppingCartItemByID", nil
	}
	return "", fmt.Errorf("%w for ShoppingCartItem due to %v", ErrTypeNotFound,
