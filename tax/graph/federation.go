...
func (ec *executionContext) resolveEntity(
	...
	switch typeName {
	case "TaxRate":
		resolverName, err := entityResolverNameForTaxRate(ctx, rep)
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "TaxRate": %w`, err)
		}
		switch resolverName {
		...
		case "findTaxRateByID":
			...
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findTaxRateByID(): %w`, err)
			}
			entity, err := ec.Resolvers.Entity().FindTaxRateByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "TaxRate": %w`, err)
			}

			return entity, nil
		}
	case "TaxRateVersion":
		resolverName, err := entityResolverNameForTaxRateVersion(ctx, rep)
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "TaxRateVersion": %w`, err)
		}
		switch resolverName {
		...
		case "findTaxRateVersionByID":
			id0, err := ec.unmarshalNUUID2githubᚗcomᚋgoogleᚋuuidᚐUUID(ctx, rep["id"])
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findTaxRateVersionByID(): %w`, err)
			}
			entity, err := ec.Resolvers.Entity().FindTaxRateVersionByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "TaxRateVersion": %w`, err)
			}
...
func entityResolverNameForTaxRate(ctx context.Context, rep EntityRepresentation) (string, error) {
	...
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for TaxRate", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for TaxRate", ErrTypeNotFound))
			break
		}
		return "findTaxRateByID", nil
	}
	return "", fmt.Errorf("%w for TaxRate due to %v", ErrTypeNotFound,
		errors.Join(entityResolverErrs...).Error())
}

func entityResolverNameForTaxRateVersion(ctx context.Context, rep EntityRepresentation) (string, error) {
	// we collect errors because a later entity resolver may work fine
	// when an entity has multiple keys
	entityResolverErrs := []error{}
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for TaxRateVersion", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for TaxRateVersion", ErrTypeNotFound))
			break
		}
		return "findTaxRateVersionByID", nil
	}
	return "", fmt.Errorf("%w for TaxRateVersion due to %v", ErrTypeNotFound,
		errors.Join(entityResolverErrs...).Error())
}
