...
// exactly, including the two error messages (spec §6).
//
//   - If no Authorized-User header was parsed into the context (anonymous, or a
//     header that failed to parse — e.g. an unknown role string), it fails with
//     the "not set or could not be parsed" message.
