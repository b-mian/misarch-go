...
func (ec *executionContext) resolveEntity(
	...
	case "Wishlist":
		...
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "Wishlist": %w`, err)
		}
		switch resolverName {
		...
		case "findWishlistByID":
			id0, err := ec.unmarshalNUUID2githubᚗcomᚋgoogleᚋuuidᚐUUID(ctx, rep["id"])
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findWishlistByID(): %w`, err)
			}
			entity, err := ec.Resolvers.Entity().FindWishlistByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "Wishlist": %w`, err)
			}
...
func entityResolverNameForWishlist(ctx context.Context, rep EntityRepresentation) (string, error) {
	// we collect errors because a later entity resolver may work fine
	// when an entity has multiple keys
	entityResolverErrs := []error{}
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for Wishlist", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for Wishlist", ErrTypeNotFound))
			break
		}
		return "findWishlistByID", nil
	}
	return "", fmt.Errorf("%w for Wishlist due to %v", ErrTypeNotFound,
		errors.Join(entityResolverErrs...).Error())
}
